...
```{r}
  sliderInput('sampleSize', 'Size of Training Dataset', min=100, max=min(nrow(train_dataset), TOTAL_BUDGET/BASE_COST_PER_ROW),
            value=100, step=500, round=0)

checkboxGroupInput("demographicDataToCollect", 
                              h4("Would you like to additionally collect any demographic data?"), 
                              choiceNames = list("Collect data on age (+$20/row)", 
                                             "Collect data on race (+$20/row)", 
                                             "Collect data on sex (+$20/row)"), 
                              choiceValues=list("age", "race", "sex"))
...
```{r}
renderText({
 paste('$', TOTAL_BUDGET - input$sampleSize * (BASE_COST_PER_ROW + 20 * length(input$demographicDataToCollect)))
})
```
