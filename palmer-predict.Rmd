...
### Collecting training data: do you wish to purchase demographic data for each patient (age, race, and sex)? {data-height=300}

```{r} 
actionButton("add_race_data", "Collect data on patient race (+$20/patient)")#, style='padding:4px; font-size:80%')
actionButton("add_sex_data", "Collect data on patient sex (+$20/patient)")#, style='padding:4px; font-size:80%')
actionButton("add_age_data", "Collect data on patient age (+$20/patient)")#, style='padding:4px; font-size:80%')
...
### Collecting training data: do you wish to collect data for more patients?
...
### Remaining training data budget {data-height=100}
```{r}
renderText(paste0('Total remaining budget: $', as.integer(budget$v), '. ',
                  'Total rows in dataset: ', sum(purchased_idxs$v), '. ',
                  'Cost per row of data: $', cost_per_row$v, '.'))
...
### Designing the model {data-height=300}
```{r}
checkboxGroupInput(
...
results_table = reactive({
  ...
  if(input$stratify_performance == 'Full population'){
    ...
    table["Number in training data"] = nrow(usable_train_dataset())
    table['% of training data'] = 100
  }else if(input$stratify_performance=='Age'){
    table = test_dataset %>% 
      group_by(Age) %>% 
      summarize(RMSE=sqrt(mean(err^2)))
    counts = usable_train_dataset() %>% group_by(Age) %>% summarise(`Number in training data` = n())
    counts['% of training data'] = 100 * counts['Number in training data'] / nrow(usable_train_dataset())
    table = merge(table, counts, by='Age')
    
    test_counts = test_dataset %>% group_by(Age) %>% summarise(n = n()) 
    test_counts['% of test data'] = 100 * test_counts['n'] / nrow(test_dataset)
    test_counts = test_counts %>% select(-n)
    table = merge(table, test_counts, by='Age')
    
  }else if(input$stratify_performance=='Race'){
    table = test_dataset %>% 
      group_by(Race) %>% 
      summarize(RMSE=sqrt(mean(err^2)))
    counts = usable_train_dataset() %>% group_by(Race) %>% summarise(`Number in training data` = n())
    counts['% of training data'] = 100 * counts['Number in training data'] / nrow(usable_train_dataset())
    table = merge(table, counts, by='Race')
    
    test_counts = test_dataset %>% group_by(Race) %>% summarise(n = n()) 
    test_counts['% of test data'] = 100 * test_counts['n'] / nrow(test_dataset)
    test_counts = test_counts %>% select(-n)
    table = merge(table, test_counts, by='Race')
  }
  else if(input$stratify_performance=='Sex'){
    table = test_dataset %>% 
      group_by(Sex) %>% 
      summarize(RMSE=sqrt(mean(err^2)))
    counts = usable_train_dataset() %>% group_by(Sex) %>% summarise(`Number in training data` = n())
    counts['% of training data'] = 100 * counts['Number in training data']  / nrow(usable_train_dataset())
    table = merge(table, counts, by='Sex')
    
    test_counts = test_dataset %>% group_by(Sex) %>% summarise(n = n()) 
    test_counts['% of test data'] = 100 * test_counts['n'] / nrow(test_dataset)
    test_counts = test_counts %>% select(-n)
    table = merge(table, test_counts, by='Sex')
  }else{
    stopifnot(FALSE)
  }
  table
...
renderTable({results_table()}, digits=2)
```
